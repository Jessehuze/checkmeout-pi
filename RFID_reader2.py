...
def get_items_from_db():
    ...
    if response.status_code == 200:
        ...
        item_dict = {
            12333: {"name": "drill", "id": 1},
            12344: {"name": "hammer", "id": 2},
            12222: {"name": "nails", "id": 3}
        }

    return item_dict


def get_reservations_from_db(items, all_in=False):
    """
    ...
    if response.status_code == 200:
        ...
        reservation_dict = {
            # 12333: {"checkout_time": "5:00PM", "item_id": 1},
            # 12344: {"checkout_time": "3:00PM", "item_id": 2},
            # 12222: {"checkout_time": "1:00PM", "item_id": 3},
        }

    if all_in:
        all_items = items.keys()
        send_request("in", 12345, all_items)
...
def main():
    ...
    reservations = get_reservations_from_db(items, all_in=True)
    ...
    while True: # endless loop
        ...
        while True:
            ...
            if scanned_items:
                results = send_request(in_out, user_id, scanned_items)
                # reservations = get_reservations_from_db(items)
                for tag_id in results["success"]:
                    # if tag_id in reservations.keys():
                    if in_out == "in":
                        del reservations[tag_id]
                    else:
                        reservations[tag_id] = {"checkout_time": "5:00PM", "item_id": items[tag_id]["id"]},
            else:
