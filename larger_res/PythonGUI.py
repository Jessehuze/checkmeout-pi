...
# disable linting warnings, don't remove
...
main_canvas = tk.Canvas(frame, bg="white", width=1700, height=1000)
main_canvas.pack()
...
# Check Out button
checkOutButton = tk.Button(root, text="Check Out", command=checkout, highlightthickness=0, bd=0, activebackground='white', cursor='none')
checkOutButton.pack()
checkOutButton.place(x=660, y=800, width=380, height=130)
checkOutImg = ImageTk.PhotoImage(file='checkout_button.png')
checkOutButton.configure(image=checkOutImg, bg='white')

# Check In button
checkInButton = tk.Button(root, text="Check In", command=checkin, highlightthickness=0, bd=0, activebackground='white', cursor='none')
checkInButton.pack()
checkInButton.place(x=660, y=600, width=380, height=130)
checkInImg = ImageTk.PhotoImage(file='checkin_button.png')
checkInButton.configure(image=checkInImg, bg='white')

# Home button from Check In
homeButtonIn = tk.Button(root, text="Home", command=goHomefromIn, highlightthickness=0, bd=0, activebackground='white', cursor='none')
homeImageIn = ImageTk.PhotoImage(file='checkin_button.png')
homeButtonIn.configure(image=homeImageIn, bg='white')

#Home button from Check Out
homeButtonOut = tk.Button(root, text="Home", command=goHomefromOut, highlightthickness=0, bd=0, activebackground='white', cursor='none')
homeImageOut = ImageTk.PhotoImage(file='checkout_button.png')
